--4a02bd3eca4ea2489c60f71c0a554a867d441885b6c3213598c7788af145
...
var rateLimitMiddleware = /* @__PURE__ */ __name((limit = 60, windowSeconds = 60) => {
  return async (c, next) => {
    ...
    const key = user ? `rl:user:${user.id}` : `rl:ip:${c.req.header("CF-Connecting-IP") || "unknown"}`;
...
var OracleClient = class {
  ...
  async fetch(url, options = {}) {
    const headers = {
      "Authorization": `Bearer ${this.env.ORACLE_API_TOKEN}`,
      ...options.headers
    };
    return globalThis.fetch(url, { ...options, headers });
  }
  async request(path, options = {}) {
    const url = `${this.env.ORACLE_API_BASE}${path}`;
    const headers = {
      "Authorization": `Bearer ${this.env.ORACLE_API_TOKEN}`,
      "Content-Type": "application/json",
      ...options.headers
    };
    const response = await globalThis.fetch(url, { ...options, headers }).catch((err) => {
      console.error("Tunnel fetch failed, using fallback:", err);
      return null;
    });
    if (!response) {
      return { rows: [], error: "Internal API Gateway Offline" };
    }
    if (!response.ok) {
...
__name(OracleClient, "OracleClient");
function getOracleClient(env) {
  return new OracleClient(env);
}
__name(getOracleClient, "getOracleClient");

// src/routes/health.ts
var health = new Hono2();
health.get("/", async (c) => {
  const oracle = new OracleClient(c.env);
  let dbStatus = "unreachable";
  try {
    const rows = await oracle.query("SELECT 1 as active");
...
// src/routes/trims.ts
var trims = new Hono2();
trims.get("/:slug", async (c) => {
  const slug = c.req.param("slug");
  const oracle = new OracleClient(c.env);
  const querySql = `
    SELECT t.*, m.name as model_name, b.name as brand_name 
    FROM trims t 
    JOIN models m ON t.model_id = m.id 
    JOIN brands b ON m.brand_id = b.id 
    WHERE t.slug = :slug
  `;
  const rows = await oracle.query(querySql, { slug });
  if (rows.length === 0) {
    return c.json({ error: "Trim not found" }, 404);
  }
  const trim = rows[0];
  if (typeof trim.specs === "string")
    trim.specs = JSON.parse(trim.specs);
  if (typeof trim.metadata === "string")
    trim.metadata = JSON.parse(trim.metadata);
  return c.json({ data: trim });
});
trims.get("/model/:modelSlug", async (c) => {
  ...
  `;
  const rows = await oracle.query(querySql, { modelSlug });
  const parsedTrims = rows.map((t) => {
    if (typeof t.specs === "string")
      t.specs = JSON.parse(t.specs);
    if (typeof t.metadata === "string")
      t.metadata = JSON.parse(t.metadata);
    return t;
  });
  return c.json({ data: parsedTrims });
});
var trims_default = trims;

// src/routes/media.ts
var media = new Hono2();
media.get("/:entityType/:entityId", async (c) => {
  ...
  `;
  const rows = await oracle.query(querySql, { entityType, entityId });
  const parsedMedia = rows.map((m) => {
    if (typeof m.metadata === "string")
      m.metadata = JSON.parse(m.metadata);
    return m;
  });
  return c.json({ data: parsedMedia });
});
...
var AIRouter = class {
  ...
  async embed(text) {
    ...
    try {
      ...
      return response.data[0] || new Array(768).fill(0.1);
    } catch (e) {
      console.error("Workers AI Embedding failed:", e);
      return new Array(768).fill(0.1);
    }
  }
  async chatStream(messages, callback) {
    if (!this.env.AI) {
      callback("Chatbot AI service offline. Please try again later.");
      return;
    }
    try {
      ...
      console.error("Workers AI Chat stream failed:", e);
      callback("Error processing chat response.");
    }
  }
};
...
search.get("/", async (c) => {
  ...
  const oracle = new OracleClient(c.env);
  const ai = new AIRouter(c.env);
  if (!queryStr) {
    return c.json({ data: [] });
  }
  let rows = [];
  if (mode === "vector" || mode === "hybrid") {
    const embedding = await ai.embed(queryStr);
    const pgVectorStr = `[${embedding.join(",")}]`;
    const vectorSql = `
      SELECT v.entity_id, v.entity_type, v.content, 
             1 - (v.embedding <=> :vector::vector) as similarity,
             t.name as trim_name, t.price_ex_showroom_base, m.name as model_name, b.name as brand_name
      FROM vector_docs v
      LEFT JOIN trims t ON v.entity_id = t.id AND v.entity_type = 'trim'
      LEFT JOIN models m ON t.model_id = m.id
      LEFT JOIN brands b ON m.brand_id = b.id
      ORDER BY v.embedding <=> :vector::vector
      LIMIT :limit
    `;
    rows = await oracle.query(vectorSql, { vector: pgVectorStr, limit });
  } else {
    const textSql = `
      SELECT t.id as entity_id, 'trim' as entity_type, t.name as trim_name, 
             t.price_ex_showroom_base, m.name as model_name, b.name as brand_name
      FROM trims t
      JOIN models m ON t.model_id = m.id
      JOIN brands b ON m.brand_id = b.id
      WHERE t.name ILIKE :query OR m.name ILIKE :query OR b.name ILIKE :query
      LIMIT :limit
    `;
    rows = await oracle.query(textSql, { query: `%${queryStr}%`, limit });
  }
...
var isOldBunVersion = /* @__PURE__ */ __name(() => {
  const version5 = typeof Bun !== "undefined" ? Bun.version : void 0;
  if (version5 === void 0) {
    return false;
  }
...
// ../../node_modules/.pnpm/hono@4.12.23/node_modules/hono/dist/helper/streaming/stream.js
...
chat.post("/", async (c) => {
  ...
  const contextSql = `
    ...
    LIMIT 3
  `;
  const contextRows = await oracle.query(contextSql, { vector: pgVectorStr });
  const contextText = contextRows.map((r) => r.content).join("\n\n");
  const systemPrompt = `You are AutoPulse's intelligent automotive chatbot helper.
Use the following context to answer the user's questions about car brands, models, variants, specifications, regional on-road prices, EV specs, and comparisons.
If the information is not in the context, use your general automotive knowledge, but clearly state what is sourced vs estimated. Speak in an expert, helpful tone.
...
`;
  ...
  return stream(c, async (stream2) => {
    await ai.chatStream(finalMessages, (chunk) => {
      stream2.write(`data: ${JSON.stringify({ text: chunk })}

`);
    });
    stream2.write("data: [DONE]\n\n");
  });
});
...
// ../../node_modules/.pnpm/@supabase+functions-js@2.107.0/node_modules/@supabase/functions-js/dist/module/helper.js
...
// ../../node_modules/.pnpm/@supabase+functions-js@2.107.0/node_modules/@supabase/functions-js/dist/module/FunctionsClient.js
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/lib/websocket-factory.js
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/lib/serializer.js
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/lib/transformers.js
var PostgresTypes;
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/phoenix/presenceAdapter.js
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/RealtimePresence.js
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/lib/normalizeChannelError.js
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/phoenix/channelAdapter.js
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/RealtimeChannel.js
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/phoenix/socketAdapter.js
...
// ../../node_modules/.pnpm/@supabase+realtime-js@2.107.0/node_modules/@supabase/realtime-js/dist/module/RealtimeClient.js
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/lib/base64url.js
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/GoTrueAdminApi.js
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/lib/local-storage.js
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/lib/locks.js
var internals = {
  /**
   * @experimental
   */
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/lib/polyfills.js
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/lib/web3/ethereum.js
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/lib/webauthn.errors.js
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/lib/webauthn.js
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/GoTrueClient.js
polyfillGlobalThis();
var DEFAULT_OPTIONS = {
  url: GOTRUE_URL,
...
// ../../node_modules/.pnpm/@supabase+auth-js@2.107.0/node_modules/@supabase/auth-js/dist/module/AuthClient.js
...
var verifyAuth = authMiddleware();

// src/routes/admin.ts
var admin = new Hono2();
admin.use("*", verifyAuth);
async function proxyToOracle(c, path, init = {}) {
  const oracle = getOracleClient(c.env);
  const url = `${c.env.ORACLE_API_BASE}/admin${path}`;
  const authHeader = c.req.header("Authorization");
  const headers = {
  ...
  try {
    const upstream = await oracle.fetch(url, {
      method: init.method || c.req.method,
      headers,
      body: init.body || c.req.method !== "GET" && c.req.method !== "HEAD" ? await c.req.text() : void 0,
      // CRITICAL: CF Workers default to buffering. We need streaming for SSE.
      redirect: "manual"
    });
    return new Response(upstream.body, {
      ...
      headers: {
        ...
        // Disable Cloudflare/NGINX buffering
    ...
    console.error("Oracle Proxy Error:", e);
    return c.json({ error: "Internal API returned 530", details: e.message }, 530);
  }
}
__name(proxyToOracle, "proxyToOracle");
admin.get("/metrics", (c) => proxyToOracle(c, "/metrics"));
admin.get("/pipeline/stream", (c) => proxyToOracle(c, "/pipeline/stream", {
  headers: { "Accept": "text/event-stream" }
}));
admin.post("/pipeline/trigger", (c) => proxyToOracle(c, "/pipeline/trigger"));
admin.get("/quarantine", (c) => proxyToOracle(c, "/quarantine"));
...
app.route("/trims", trims_default);
app.route("/api/trims", trims_default);
...
app.get("/", (c) => {
  return c.json({
    message: "AutoPulse Edge Gateway API v2.0",
    endpoints: ["/health", "/trims/:slug", "/media/:entityType/:entityId", "/search?q=...", "/chat"]
  });
});
var src_default = app;
export {
